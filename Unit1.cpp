...
{
	if (Edit2->Text == "")
		ShowMessage("Нажмите сначала кнопку \"ПЕРЕВЕСТИ\"");
	else
	{
		char variable;
		//общий массив значений переменных (начальные значения из таблицы)
		float array[201];
		for (int i = 1; i < 6; i++)
		{
			variable = StringGrid1->Cells[0][i][1]; //имя переменной из таблицы
			//значения переменных из таблицы записать в массив
			array[int(variable)] = StrToFloat(StringGrid1->Cells[1][i]);
		}
		//вывод в Edit3 подсчета по постфиксной форме (округление до 2 знаков)
		Edit3->Text = FloatToStrF(opz->Rezult(Edit2->Text, array), ffFixed,7,2);
		float rez;
		if (Edit1->Text == "(a+b)*(c-d)/e")
		{
	//проверка подсчета по инфиксной форме и вывод в Edit4 (округление до 2 знаков)
